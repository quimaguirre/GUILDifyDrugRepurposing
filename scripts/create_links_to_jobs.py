...
def main():
    ...
    type_job='drug' # disease / drug
    place='localhost' # localhost / aleph
    ...
    if place=='localhost':
        repur_path = '/var/www/html/guildify2/data/drug_repurposing'
    ...
    for identifier in id_to_assoc:
        ...
        if type_job=='disease':
            id2 = id_to_id2[identifier]
            link_dir = os.path.join(sessions_dir, id2)
        else:
            link_dir = os.path.join(sessions_dir, identifier)

        if not os.path.exists(input_dir):
            print('{} and {} does not exists'.format(identifier, job))
        if not os.path.exists(link_dir):
            command = 'ln -s {} "{}"'.format(input_dir, link_dir)
            print(command)
            os.system(command)
